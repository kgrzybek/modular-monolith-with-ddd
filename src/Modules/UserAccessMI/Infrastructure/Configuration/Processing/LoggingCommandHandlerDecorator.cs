...
internal class LoggingCommandHandlerDecorator<T> : ICommandHandler<T>
    where T : Application.Contracts.ICommand
{
    ...
    public async Task Handle(T command, CancellationToken cancellationToken)
    {
        ...
        using (
            LogContext.Push(
                new LoggingCommandHandlerDecorator<T>.RequestLogEnricher(_executionContextAccessor),
                new LoggingCommandHandlerDecorator<T>.CommandLogEnricher(command)))
        {
            try
            {
    ...
    {
        private readonly Application.Contracts.ICommand _command;

        public CommandLogEnricher(Application.Contracts.ICommand command)
        {
            _command = command;
        }
